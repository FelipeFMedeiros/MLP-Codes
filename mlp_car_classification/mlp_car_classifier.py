...
def carregar_e_preprocessar_dados(path_csv: str) -> Tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    print(f"Ranges normalizados: Ano [{year_min}-{year_max}], Preço [${price_min:,}-${price_max:,}], KM [{mileage_min:,}-{mileage_max:,}]")
...
def main():
    ...
    for epoca in range(epocas):
        ...
        # Mostrar cálculo detalhado na primeira época
        if epoca == 0:
            # Refazer propagação para o exemplo
            camada_oculta_ex = np.zeros(5)
            for i in range(5):
                soma = 0
                for j in range(4):
                    soma += linha_exemplo[j] * pesos_camada_oculta[i][j]
                camada_oculta_ex[i] = sigmoid(soma)
            
            camada_saida_ex = np.zeros(2)
            for i in range(2):
                camada_saida_ex[i] = pesos_camada_saida[i][0]
                for j in range(1, 6):
                    camada_saida_ex[i] += camada_oculta_ex[j-1] * pesos_camada_saida[i][j]
            
            print(f"Camada oculta após propagação: {camada_oculta_ex}")
            print(f"Camada de saída: {camada_saida_ex}")
            print(f"Saída esperada: {y[exemplo_idx]}")
    ...
    for _, car in premium_examples.iterrows():
        print(f"      • {car['make']:12} {car['model']:15} {car['year']} - ${car['price']:6,} ({car['mileage']:,} milhas)")
